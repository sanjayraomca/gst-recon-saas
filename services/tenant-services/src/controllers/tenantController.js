    ...
    try {
        ...
        if (type && type !== 'ALL') {
            ...
            if (typeLower === 'notices') {
                query = query.whereIn(database.raw('LOWER(activity_logs.entity_type)'), ['notice', 'gst_notice']);
            } else if (typeLower === 'tax liabilities') {
                query = query.whereIn(database.raw('LOWER(activity_logs.action_type)'), ['liability_due', 'payment_pending']);
            } else if (typeLower === 'user actions') {
                query = query.whereIn(database.raw('LOWER(activity_logs.entity_type)'), ['user', 'auth']);
            } else {
                let actionTypes = [typeLower];
                if (typeLower === 'reconciliation_run' || typeLower === 'reconciliation') {
                    actionTypes = ['reconciliation_run', 'reconciliation'];
                } else if (typeLower === 'user_login' || typeLower === 'user sign ins' || typeLower === 'login') {
                    actionTypes = ['user_login', 'login_success', 'login'];
                } else if (typeLower === 'user_invited' || typeLower === 'user invitations') {
                    actionTypes = ['user_invited', 'user_invitation'];
                } else if (typeLower === 'tally_sync_pull' || typeLower === 'tally sync') {
                    actionTypes = ['tally_sync_pull', 'tally_sync'];
                } else if (typeLower === 'delete_book_data_row' || typeLower === 'deletions') {
                    actionTypes = ['delete_book_data_row', 'delete_book_data'];
                }
                query = query.whereIn(database.raw('LOWER(activity_logs.action_type)'), actionTypes);
            }
        }
