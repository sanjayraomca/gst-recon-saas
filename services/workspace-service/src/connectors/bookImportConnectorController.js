...
const mapPurchaseRecord = (record, tenantId, workspaceId, returnPeriod) => {
    ...
    const items = (record.items || []).map(item => ({
        ...
        row_total: parseFloat(item.row_total || item.total_value || item.total_amount_with_tax || 0),
        invoice_amount: parseFloat(record.invoice_amount || record.total_value || record.net_amount || 0),
