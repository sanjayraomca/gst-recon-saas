...
const logThirdPartySync = async (req, status, respBody, options = {}) => {
    try {
        ...
        console.error('[logThirdPartySync] Failed to write inbound sync log:', err.message);
    }
};
...
const syncThirdPartyPurchases = async (req, res) => {
    try {
        ...
        for (const record of rawVouchers) {
            ...
            let netAmount = parseFloat(record.row_wise_total_amount || record.net_amount || record.total_value || record.invoice_amount || 0);
        ...
        await logThirdPartySync(req, 'success', {
            success: true,
            data: successData,
            message: 'Purchase data synchronized successfully and reconciliations fired'
        }, {
            platform,
            workspaceId: workspace.id,
            tenantId: workspace.tenant_id,
        ...
        return successResponse(res, successData, 'Purchase data synchronized successfully and reconciliations fired');
