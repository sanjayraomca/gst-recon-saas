...
class ReconciliationModel {
    ...
    static async getRunResults(workspaceId, runId, filters = {}) {
        ...
        // Join with unique supplier info to prevent row duplication
        const uniqueSuppliers = knex('supplier_master')
            .select('gstin', 'workspace_id')
            .min('supplier_name as supplier_name')
            .min('email as email')
            .groupBy('gstin', 'workspace_id')
            .as('sm');

        query = query.leftJoin(uniqueSuppliers, function () {
            if (is2aVs2b) {
        ...
        if (parsedColumnFilters && typeof parsedColumnFilters === 'object') {
            ...
            const colMapping = {
                ...
                supplier_gstin: { cols: is2aVs2b ? ['sa.supplier_gstin', 'gi.supplier_gstin'] : ['pi.supplier_gstin', 'gi.supplier_gstin'], exact: true },
                name: { cols: is2aVs2b ? ['sa.supplier_name', 'gi.supplier_name'] : ['pi.supplier_name', 'gi.supplier_name'] },
                supplier_name: { cols: is2aVs2b ? ['sa.supplier_name', 'gi.supplier_name'] : ['pi.supplier_name', 'gi.supplier_name'] },
                ...
                gstr_invoice_number: { cols: is2aVs2b ? ['sa.document_number_clean', 'gi.document_number_clean'] : ['pi.supplier_invoice_no', 'gi.document_number_clean'] },
                invoice_date: { cols: is2aVs2b ? ['sa.document_date', 'gi.document_date'] : ['pi.supplier_invoice_date', 'gi.document_date'], dateCol: true },
                gstr_invoice_date: { cols: is2aVs2b ? ['sa.document_date', 'gi.document_date'] : ['pi.supplier_invoice_date', 'gi.document_date'], dateCol: true },
                ...
                return_period: { cols: is2aVs2b ? ['gi.return_period'] : ['tp.period_code', 'gi.return_period'] },
                ...
                purchase_invoice_total: { cols: is2aVs2b ? ['sa.document_value'] : ['pi.net_amount'], numeric: true },
                ...
                purchase_taxable: { cols: [knex.raw(`COALESCE(${is2aVs2b ? 'sa.taxable_value' : (is2a ? 'ps.taxable_total' : 'pi.taxable_total')}, 0)`)], numeric: true },
                booksTaxRate: {
                    cols: [knex.raw(`
                    CASE 
        ...
        const totalsQuery = query.clone()
            ...
            .select(
                ...
                knex.raw('COUNT(DISTINCT rr.id) as total')
            );
        const totalsResult = await totalsQuery.first() || {
            gstr2b_taxable_total: 0,
        ...
        const sortMapping = {
            ...
            'tax': knex.raw(`CASE WHEN COALESCE(gi.igst, 0) > 0 OR COALESCE(${is2aVs2b ? 'sa.igst' : (is2a ? 'ps.total_igst_amount' : 'pi.total_igst_amount')}, 0) > 0 THEN 'IGST' ELSE 'CGST/SGST' END`),
            'gstType': is2aVs2b ? 'sa.source_section' : knex.raw('COALESCE(pi.source_section, gi.source_section)'),
            'invoiceRef': is2aVs2b ? 'sa.document_number_clean' : 'purchase_invoice_number',
            ...
            // Portal (Source B)
            'gstrInvoiceDate': knex.raw('COALESCE(gi.document_date, gi.isd_document_date, gi.boe_date)'),
            'gstr_invoice_date': knex.raw('COALESCE(gi.document_date, gi.isd_document_date, gi.boe_date)'),
            'gstrInvoiceAmt': knex.raw('COALESCE(gi.document_value, gi.taxable_value + COALESCE(gi.igst,0) + COALESCE(gi.cgst,0) + COALESCE(gi.sgst,0) + COALESCE(gi.cess,0))'),
            ...
            'return_period': 'gi.return_period',

            // Book Data (Source A)
            'booksInvoiceDate': is2aVs2b ? 'sa.document_date' : knex.raw('COALESCE(pi.supplier_invoice_date, pi.book_vchr_date)'),
            'booksInvoiceAmt': is2aVs2b ? 'sa.document_value' : 'purchase_invoice_total',
            'booksTaxableAmt': is2aVs2b ? 'sa.taxable_value' : 'purchase_taxable',
            ...
            'booksVoucherNo': is2aVs2b ? 'sa.document_number_clean' : 'pi.book_vchr_no',
            'gstrType': is2aVs2b ? 'sa.source_section' : knex.raw('COALESCE(pi.gstr_category, pi.source_section, gi.source_section)'),
            'taxPeriod': is2aVs2b ? 'gi.return_period' : knex.raw('COALESCE(tp.period_code, gi.return_period)'),

            // Status & Diff
            'status': 'rr.match_status',
            'diff': knex.raw('ABS(rr.variance_amount)'),
            'difference': 'rr.variance_amount',
            'tax_diff': 'rr.variance_amount',
            'action': 'rr.action_required',
    ...
    static async getRunTaxSummary(workspaceId, runId, filters = {}) {
        ...
        const invoices = await knex('reconciliation_results as rr')
            ...
            .select(
                ...
                'pi.total_cess_amount as books_cess',
                ...
                // Portal Data (Aliased for frontend compatibility)
                knex.raw("COALESCE(gi.igst,0)+COALESCE(gi.cgst,0)+COALESCE(gi.sgst,0)+COALESCE(gi.cess,0) as gstr2b_tax"),
                knex.raw("COALESCE(gi.igst,0)+COALESCE(gi.cgst,0)+COALESCE(gi.sgst,0)+COALESCE(gi.cess,0) as gstr_tax"),
                ...
                'gi.return_period as gstr_return_period',
                'gi.source_section as supplier_gst_type',
