...
class Reconciliation2AModel {
    ...
    static async getRunResults(workspaceId, runId, filters = {}) {
        ...
        const totalsResult = await query.clone().clearSelect().select(
            ...
            knex.raw('SUM(COALESCE(pi.taxable_total, sa.taxable_value, gb.taxable_value, 0)) as book_taxable_total'),
            knex.raw('SUM(COALESCE(gi.taxable_value, sa.taxable_value, 0)) as gstr_taxable_total'),
            knex.raw('SUM(COALESCE(pi.total_igst_amount,0)+COALESCE(pi.total_cgst_amount,0)+COALESCE(pi.total_sgst_amount,0) + COALESCE(sa.total_tax, 0) + COALESCE(gb.total_tax, 0)) as book_tax_total'),
            knex.raw('SUM(COALESCE(gi.total_tax, sa.total_tax, 0)) as gstr_tax_total'),
            knex.raw('SUM(COALESCE(pi.total_cgst_amount, sa.cgst, gb.cgst, 0)) as book_cgst_total'),
            knex.raw('SUM(COALESCE(pi.total_sgst_amount, sa.sgst, gb.sgst, 0)) as book_sgst_total'),
            knex.raw('SUM(COALESCE(pi.total_cess_amount, sa.cess, gb.cess, 0)) as book_cess_total'),
        ...
        query.select(
            ...
            knex.raw('COALESCE(pi.total_igst_amount, gb.igst) as purchase_igst'),
            knex.raw('COALESCE(pi.total_cgst_amount, gb.cgst) as purchase_cgst'),
            knex.raw('COALESCE(pi.total_sgst_amount, gb.sgst) as purchase_sgst'),
            knex.raw('COALESCE(pi.total_cess_amount, gb.cess) as purchase_cess'),
            knex.raw('COALESCE(pi.net_amount, gb.total_tax + gb.taxable_value) as purchase_invoice_total'),
            ...
            knex.raw('COALESCE(pi.place_of_supply, gb.place_of_supply) as purchase_pos'),
